...
export default function PCBuilder() {
  ...
  const manipulateCategories = categories.map((category) =>{
    const find = pcBuilderComponents.find((item) => item.category.toLowerCase() === category.category.toLowerCase())
  ...
  return (
    <div>
      ...
      <main
        className={`min-h-screen w-[70%] m-auto border p-4`}
      >
        <div className="border px-2 py-3 flex items-center justify-between rounded-sm mb-5">
          ...
          <Button disabled={pcBuilderComponents?.length !== 5} className="bg-blue-500 text-white" type="link">Complete Build</Button>
        </div>
        <Row gutter={[24, 24]}>
        {manipulateCategories.map((item) => (
          <Col span={12} key={item.id}>
            ...
            <div className="shadow p-4 mt-2 flex items-center justify-between border border-blue-400">
             ...
             <h3 className="text-2xl text-green-500">Tk. {item.price}</h3>
            </div>
            </>}
            </Link>
          </Col>
